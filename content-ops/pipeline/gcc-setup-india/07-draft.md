...
A standard, non-IFSC GCC entity is taxed as any other Indian Private Limited company — most GCCs incorporated as new entities opt into the concessional corporate tax regime available to domestic companies, which brings the effective tax rate down from the standard rate, subject to giving up certain exemptions and deductions. GST registration is mandatory once the entity crosses the applicable turnover threshold, or immediately if the GCC is billing its foreign parent for services (export-of-services treatment under GST has its own specific documentation requirements, which are worth getting right from the first invoice rather than retrofitting later). GST is rarely covered in competitor content on this topic for a standard GCC entity — it shouldn't be treated as an afterthought.
...
1. **NASSCOM/Zinnov 2026 market statistics** (2,100+ GCCs, $70B+ revenue, 2.4M+ employees) — sourced from Stage 1's search snippets only, not a primary NASSCOM/Zinnov report. Confirm current figures and exact attribution (which specific report/date) before publishing, and update if newer data has since been released.
2. **FC-GPR filing deadline** — stated in this draft only as "within a defined window of the share allotment date" (deliberately not naming a specific number of days since Stage 5/6 flagged this needs primary-source confirmation). If Stage 9 confirms the RBI Master Direction timeline (commonly cited elsewhere as 30 days), the draft should be updated to state it explicitly, since specificity here is the page's core differentiator.
3. **FC-TRS filing deadline** — same treatment as above; commonly cited elsewhere as 60 days from transfer, not stated as a hard number in this draft pending verification.
...
5. **Safe harbour margin threshold (the 15.5% figure and any Rs 2,000 crore turnover threshold)** — deliberately not stated as a specific number anywhere in this draft, including in the worked example (used "mid-teens percentage range" as a placeholder). Per Stage 5/6's explicit instruction, this must be verified against the actual CBDT/Income Tax Department notification — not against the existing `/blog/india-safe-harbour-rules-2026` post — before any specific percentage is added to this page.
6. **Corporate tax rate for a standard GCC entity** — referred to only generally ("the concessional corporate tax regime available to domestic companies... brings the effective tax rate down from the standard rate") without naming Section 115BAA or a specific percentage (commonly cited elsewhere as 22% base / ~25.17% effective). Confirm the section number and current effective rate before adding specifics.
7. **GST registration threshold** — referenced generically ("once the entity crosses the applicable turnover threshold") without stating the actual rupee threshold, since GST registration thresholds and export-of-services treatment have specific current figures that should be confirmed rather than assumed.
8. **PF/ESI headcount and wage-ceiling thresholds** — referenced generically in the compliance calendar ("once headcount crosses the applicable thresholds") without naming the specific employee-count triggers (commonly cited elsewhere as 20+ for PF, 10+ for ESI) or wage ceilings, pending confirmation.
9. **SPICe+ form name and Companies Act references** — SPICe+ is named as the integrated MCA incorporation form per Stage 1/5's evidence; confirm this is still the current form name/version and that no specific Companies Act section numbers were inadvertently implied anywhere in the incorporation section (none were deliberately included, but worth a final check).
10. **GIFT City/IFSCA regulatory claims** — kept deliberately general (unified regulator replacing fragmented RBI/SEBI/IRDAI oversight, distinct tax treatment) per Stage 5's explicit flag that AU Corporate's hands-on GIFT City experience is unconfirmed. Do not add any claim implying AU Corporate has executed a GIFT City structuring engagement unless the human confirms this separately — the current draft avoids this entirely.
11. **DTAA/withholding claims** — kept general and pointed to the existing `/india-entry-for-uk-companies/india-uk-dtaa-withholding-tax` and `/blog/mail-box-dtaa-benefits` pages rather than stating any rate directly on this page; confirm those linked pages' figures are themselves current, since this page inherits their credibility by reference.
12. **Form 3CEB / transfer pricing documentation threshold** — mentioned in the compliance calendar as applying to international related-party transactions "above the applicable threshold" without stating the specific rupee threshold; confirm before publish if a specific figure is wanted.
13. **BOT/JV sub-model description** ("minority-equity-retained joint venture" vs. "virtual/managed captive") — carried from Stage 4's competitor research (KNM, ANSR) as a general description of how the market categorizes BOT sub-models, not an AU Corporate-verified taxonomy. Worth a light sanity check that this terminology is accurate and not overstated.
