...
const gccVsComparisons = [
  ...
  {
    title: 'GCC vs GIC (Global In-House Center) — Is There a Real Difference?',
    body: 'Global In-House Center (GIC) is the older term for the same underlying structure a GCC describes today — a captive, wholly-owned unit an MNC sets up in India to serve its own operations rather than external clients. GIC (and, going further back, "captive center") was the more common label through the 2000s and early 2010s, when most of these centers ran fairly narrow, cost-driven back-office and IT-support functions. "GCC" became the industry-standard term as these centers matured into broader, higher-value functions — R&D, product engineering, enterprise finance, data and analytics — with genuine decision-making authority devolved from the parent, rather than pure delivery capacity.\n\nLegally and structurally, setting up a "GIC" and setting up a "GCC" in India involve the exact same entity-formation, FEMA/RBI, and tax mechanics covered throughout this guide — the terms describe the same category of captive entity, not two different legal structures. NASSCOM, Zinnov, and most current industry research now use "GCC" almost exclusively, but if this topic keeps surfacing both terms, treat "GIC" and "captive center" as interchangeable with "GCC" for setup purposes.',
  },
]
...
const entityOptions = [
  ...
  },
  {
    ...
    body: 'Under a BOT model, a third-party operator builds and initially runs the GCC — recruiting, infrastructure, and day-to-day management — with a contractual path for the foreign parent to take over full ownership and operational control after an agreed period. Some operators market this as "GCC-as-a-Service" or a "turnkey/managed GCC" rather than BOT — the label varies by vendor, but it is the same underlying model: a third party runs the center on your behalf for a period, with a defined handover point. This can shorten time-to-first-hire meaningfully, at the cost of higher ongoing fees during the build phase and a transition event you’ll need to manage carefully. AU Corporate does not operate as a BOT/managed-GCC/GCC-as-a-Service provider; our role in this model, where clients use one, is on the legal entity, tax, and compliance side, not building or running the center itself.',
  },
  {
    title: 'EOR-First',
...
const faqs = [
  ...
  { q: 'What’s the difference between a GCC and a GIC (Global In-House Center)?', a: 'None, structurally. GIC ("Global In-House Center," and before that "captive center") is the older term for the same category of entity a GCC describes today. The label shifted as these centers matured from narrow, cost-driven back-office functions into broader R&D, engineering, and enterprise-function roles with real decision-making authority — but setting up a "GIC" and setting up a "GCC" involve the exact same entity-formation, FEMA/RBI, and tax mechanics.' },
  ...
  { q: 'What does the organizational structure of a GCC in India typically look like?', a: 'Most GCCs run as a captive entity with its own India leadership (a Country Head or India CEO), functional leads for each capability delivered, and a local HR/finance/compliance function — with functional leads reporting in a matrixed way to both India leadership and their global counterpart at the parent, and governance running through a board plus a regular steering/KPI cadence with the parent’s global business units. Where a BOT/managed model is used, the operator typically builds and runs this governance layer during the build phase, transferring it to the parent at an agreed point.' },
  ...
  { q: 'Which are the best cities for a GCC in India?', a: 'Bengaluru, Hyderabad, Pune, Chennai, the NCR/Gurgaon region, and Mumbai remain the established hubs, each with different sector strengths, alongside a growing set of Tier-2 cities (Coimbatore, Kochi, Jaipur, Ahmedabad) being evaluated for cost and talent-retention reasons. Bengaluru remains the single largest hub by concentration of GCCs, per Zinnov’s city-level research.' },
  { q: 'Are there state government incentives for setting up a GCC in India, beyond the national FDI and tax rules?', a: 'Increasingly, yes. Several states now run their own dedicated GCC policies layering additional incentives on top of national FDI/tax rules — Karnataka and Haryana have named GCC policies, Tamil Nadu runs a GCC-specific incentive scheme with its own eligibility criteria, and other states are following. A national framework intended to standardize this across states has been under discussion since the Union Budget 2025-26 but is not yet finalized. Because state incentives change quickly, confirm current status and eligibility for your specific state directly before relying on any figure you’ve seen quoted elsewhere.' },
    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <p className="text-gray-600 leading-relaxed mb-10">
            That single distinction — captive versus vendor — is what separates a GCC from almost every other India-entry structure a foreign company might consider, and it&apos;s worth being precise about it before going further. You&apos;ll also see this same structure referred to as a Global In-House Center (GIC) or simply a &ldquo;captive center&rdquo; — older labels for the same category of entity, covered in the comparison below.
          </p>
      ...
      <section className="py-20 bg-secondary/50">
        <div className="max-w-5xl mx-auto px-4">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-4">GCC Organizational Structure: How the India Team Is Actually Governed</h3>
          <p className="text-gray-600 leading-relaxed mb-4">
            Entity structure (WOS, branch, BOT, EOR) answers a legal question — how the India team is legally constituted. Organizational structure answers a different, equally important question once that entity exists: how the India center is actually run day to day, and who it answers to.
          </p>
          <p className="text-gray-600 leading-relaxed mb-4">
            Most GCCs settle into one of a few governance patterns. In a fully captive model — the default for a WOS — the India entity has its own leadership (typically a Country Head or India CEO), functional leads for each capability the center delivers (engineering, finance, analytics, and so on), and a local HR/finance/compliance function, with each functional lead reporting in a matrixed way both to India leadership and to their functional counterpart at the global parent. Governance runs through a board (as required under the Companies Act for any Private Limited entity) plus, in practice, a regular operating cadence — steering committees, KPI reviews, roadmap planning — jointly run with the parent&apos;s global business units rather than treated as a purely local decision.
          </p>
          <p className="text-gray-600 leading-relaxed mb-8">
            Where a BOT/managed model is used instead, the operator typically runs this governance layer on the foreign parent&apos;s behalf during the build phase — recruiting the leadership team, setting up the functional structure — with a defined transfer point at which the parent takes over direct governance. None of this organizational design work is legal or compliance work in the strict sense, so it sits outside what AU Corporate&apos;s regulatory compliance practice directly builds — but it&apos;s worth understanding as you make the entity-structure decision above, since a WOS with weak governance design and a BOT with a clean one can end up in a similar operational place, just by a different route.
          </p>
      ...
      <section className="py-20 bg-secondary/50">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <p className="text-gray-600 leading-relaxed mb-4">
            City choice for a GCC is genuinely a separate workstream from the legal and compliance decisions this guide focuses on. At a factual level, the established hubs remain Bengaluru, Hyderabad, Pune, Chennai, the NCR/Gurgaon region, and Mumbai, each with different sector strengths, alongside a growing set of Tier-2 cities (Coimbatore, Kochi, Jaipur, Ahmedabad) being evaluated for cost and talent-retention reasons. Bengaluru remains the single largest concentration by a wide margin — Zinnov&apos;s city-level research puts it at close to 30% of all GCCs in India — largely on the strength of its software engineering and R&D talent pool, though that concentration also means it is the most cost- and talent-competitive city to set up in. We&apos;re covering city comparison at this overview level deliberately; a fuller comparison sits outside AU Corporate&apos;s core advisory lane and is better handled alongside a location-specific real estate or workforce advisor.
          </p>
          <p className="text-gray-600 leading-relaxed mb-8">
            Facilities setup runs on its own timeline alongside the legal entity work: broadly, GCCs either negotiate a traditional multi-year commercial lease with a dedicated fit-out phase, or use a flexible/managed workspace to get a team seated within days while a longer-term facility decision is made in parallel. Either route also brings its own compliance layer — fire safety and occupancy approvals, and (for a tech-delivering GCC) information-security and data-protection controls appropriate to the function — that sits alongside, not instead of, the FEMA/RBI and tax compliance this guide focuses on.
          </p>

          <h3 className="text-xl font-semibold text-gray-900 mb-3">State GCC Policies: Karnataka, Telangana, Tamil Nadu, Haryana, and Others</h3>
          <p className="text-gray-600 leading-relaxed mb-3">
            Beyond the national FDI and tax framework this guide covers, a growing number of Indian states now run their own dedicated GCC policies, layering state-level incentives on top of the standard national rules — and this is one of the fastest-moving pieces of the GCC landscape right now, worth treating as a live item rather than a settled fact.
          </p>
          <p className="text-gray-600 leading-relaxed mb-3">
