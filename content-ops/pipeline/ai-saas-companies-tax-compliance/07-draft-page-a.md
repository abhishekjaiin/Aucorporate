...
## Registering: Form GST REG-09, an Indian Representative, and What's Needed

Once you've established that you're in scope, registration runs through the **non-resident taxable person route**, using **Form GST REG-09**. *[Writer's note: confirm with Stage 9 whether REG-09 is the correct form specifically for an OIDAR/foreign digital-services supplier as opposed to the general non-resident taxable person category — there's a real risk of conflating two adjacent but distinct registration routes here, and this needs to be checked against the current CGST Rules before publishing.]* The statutory basis for mandatory registration sits in **CGST Act, Section 24**, which lists specific categories of suppliers required to register regardless of turnover — a non-resident/OIDAR supplier is one of them.

A practical requirement that trips a lot of foreign applicants up: you need an **Indian authorized representative** to complete the registration and act as the point of contact with GST authorities on your behalf. This isn't a formality you can skip by having a director sign the form remotely — it has to be a person or entity actually present in India, which is one of the more common reasons companies bring in an advisory firm to run registration rather than attempt it entirely in-house from abroad. Our GST advisory practice handles this end to end — completing REG-09, appointing and standing in as the Indian representative where needed, and getting the registration issued — rather than a company having to work out the mechanics of appointing a local representative from scratch.
...
On the invoicing side, OIDAR supplies are classified under **SAC Code 9983**, and the applicable GST rate is **18%** — both of which need to show correctly on customer-facing invoices, not just on the internal filing. *[Writer's note: confirm SAC 9983 and the 18% rate remain current and unchanged for OIDAR supplies specifically — flag for Stage 9.]*
...
**CGST Act, Section 122** sets out the penalty framework for failing to register when required to. **CGST Act, Section 50** separately imposes interest — 18% per annum — on any GST that should have been paid and wasn't, running from when it was due, not from when the authorities catch up with you. *[Writer's note: confirm the current penalty quantum under Section 122 for this specific failure category with Stage 9 before publishing a number — the architecture brief deliberately doesn't specify one, and this page shouldn't either until it's verified.]*

There's also a more current, practical risk signal worth naming plainly: enforcement in this space has been getting more active, not less, over the past couple of years, with Indian tax authorities increasingly able to cross-reference payment-gateway and card-network data to identify overseas sellers billing Indian customers without a registration on file. *[Writer's note: this enforcement-trend claim needs a specific, checkable source before publishing — Stage 9 should either find one or this should be softened to avoid an unverifiable claim about DGGI activity specifically.]* The practical upshot is that "unregistered and undetected" is a shrinking window, not a stable position — this is a genuinely different risk landscape than it was even a couple of years ago, and treating non-registration as low-risk because it's gone unnoticed so far isn't a safe assumption to keep making.
...
The second is **Significant Economic Presence** under Section 9(9) of the Income-tax Act — a completely separate, direct-tax exposure that can apply to a foreign company with sustained, systematic transactions with Indian customers, independent of GST status entirely. It's a real consideration, but it's not this page's subject, and it's not the same regime as OIDAR — conflating the two is a mistake worth actively avoiding. Our [incorporation-timing guide](/doing-business-in-india/incorporation) covers Significant Economic Presence and the broader business-connection risk in full; this page's scope stays on GST/OIDAR.

---

## How This Differs From GST Once You Have an Indian Entity

Everything above applies specifically to a foreign company with no Indian entity. The picture changes once you incorporate — GST then follows the **ordinary domestic turnover threshold** (Rs 20 lakh for services, Rs 40 lakh for goods in most states), the same rule any other Indian business operates under, rather than the no-threshold OIDAR/NTOR rule this page describes. Our [taxation & regulatory practice](/services/taxation-regulatory) covers that domestic GST regime, along with the fuller direct tax and compliance calendar that applies once you have an entity on the ground.

The important thing to understand is that an existing OIDAR registration doesn't simply vanish the day you incorporate — it folds into the new entity's ordinary GST position, and the transition itself needs to be handled deliberately rather than assumed to happen automatically. If incorporation is realistically on your horizon, our [India entry guide for SaaS and AI companies](/india-entry-for-saas-companies) walks through exactly what changes, including this transition, in the section on GST after incorporation.
...
- Inside "Registering: Form GST REG-09..." — a short inline prompt: "We handle OIDAR registration end to end, including standing in as your Indian authorized representative — get in touch if you're ready to register."
...
Everything below needs Stage 9 (fact-authority-checker) confirmation against gst.gov.in/cbic.gov.in and the current CGST Rules/Act text before this page publishes. None of these numbers or dates were invented for this draft — each is carried forward from the approved architecture brief's own sourcing list or flagged there as needing verification — but I'm flagging them again explicitly at the point they appear in copy so nothing slips through silently:

1. **CBIC notification number and exact effective date for the October 2023 OIDAR definition amendment** — architecture doc explicitly instructs not to state an unverified number; I haven't. Needs Stage 9 to locate and confirm, or the page should keep citing it descriptively (as drafted) rather than by notification number.
2. **Form GST REG-09** — the architecture brief specifies this form for OIDAR/non-resident registration. I've flagged in the body that Stage 9 should confirm this is the correct form specifically for an OIDAR/foreign digital-services supplier (as opposed to a general non-resident taxable person registering under a different route/form) — there's a real risk of two adjacent registration categories being conflated here, and I don't have enough certainty to state it as unambiguously correct without a check.
3. **GSTR-5A due date ("20th of the following month")** — carried from the architecture brief; needs a live confirmation the deadline hasn't shifted.
4. **SAC Code 9983 and the 18% GST rate for OIDAR supplies** — carried from the architecture brief; confirm both are still current and that 9983 is the correct/complete code (vs. a narrower sub-code) for OIDAR specifically.
5. **CGST Act Section 122 — exact penalty quantum for failure to register.** I deliberately did not state a rupee figure or percentage anywhere in the draft, per the guardrail against inventing figures — this needs Stage 9 to either supply a verified number or confirm the page should stay qualitative on this point.
6. **CGST Act Section 50 — 18% per annum interest rate.** Carried from the architecture brief; confirm this is the current rate and that it applies to this specific fact pattern (unpaid IGST on OIDAR supplies) rather than a different provision.
7. **IGST Act Section 13(12) place-of-supply mechanics for the B2B reverse-charge carve-out** — confirm my plain-language description (registered recipient self-assesses, liability shifts off the foreign supplier) matches the statutory mechanism precisely; I've kept this deliberately general rather than quoting statutory language I'm not certain of.
8. **Equalisation levy withdrawal dates** — 2% levy withdrawn August 2024, 6% digital-advertising levy withdrawn April 2025 (Finance Bill 2025). These are load-bearing for the past-tense framing throughout the page and need to be exactly right, not approximate.
9. **DGGI enforcement / payment-gateway data-sharing claim** in "What Happens If You Don't Register" — this is the one claim in the draft I'd flag as needing either a real, checkable source or a deliberate softening before publication. The architecture brief calls for "a current risk signal... intensifying DGGI enforcement via payment-gateway data sharing (2025-2026)" but doesn't supply a citable source, and I don't have one either. I've written it as a general, unquantified trend statement rather than citing a specific enforcement action, dataset, or number — but Stage 9 should either find a genuine source to cite here or ask Stage 8/11 to soften it further. This is the single highest-risk unverified claim on the page.
10. **Indian authorized representative requirement** — I've described this as a practical/procedural necessity without citing a specific CGST Rule number, since I wasn't confident of the exact rule. If a precise citation is wanted here (matching the site's existing precise-inline-citation style on `taxation-regulatory`), Stage 9 should supply the rule number.
11. Per the approved architecture, I did **not** restate the specific SEP thresholds (INR 2 crore / 300,000 users) on this page — that numeric detail was intentionally left to `/doing-business-in-india/incorporation` per Stage 6's scoping instruction, and I kept this page's SEP mention to a single scoped paragraph plus a link, consistent with that instruction. Flagging only so this isn't mistaken for an oversight.
